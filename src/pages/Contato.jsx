...
const novaSubtopics = [
  { title: 'Regionalidade', text: 'A regionalidade no guia alimentar e na ficha técnica valorizam a cultura alimentar local e a sustentabilidade, como por exemplo a sazonalidade (priorização de ingredientes da época); a identidade cultural, onde deve se registrar se a preparação é típica de alguma região; o acesso e origem, considerando‑se os ingredientes que são facilmente encontrados na sua região.' },
  { title: 'Custo da Receita', text: 'O custo é o fator de viabilidade da FTP. No contexto do GAPB, o que se busca é o equilíbrio entre saúde e economia.' },
  { title: 'Custo Unitário e Total', text: 'É calculado o custo de cada ingrediente com base no Peso Bruto (PB) utilizado.' },
  { title: 'Aproveitamento Integral', text: 'Para reduzir o custo e o desperdício, a FTP pode incluir o uso de cascas, talos e sementes (alinhado com o consumo consciente).' },
  { title: 'Custo por Porção', text: 'É calculado o valor final de cada preparo. Alimentos in natura, quando comprados na safra e de produtores locais costumam ter um custo‑benefício muito superior aos processados.' },
  { title: 'Acesso aos Ingredientes', text: 'Disponibilidade nos comércios locais, facilidade de substituição e dependência da época de colheita permitem adaptar receitas para diferentes rendas.' },
  { title: 'Destinação da Preparação', text: 'As receitas podem atender a diversos públicos: alimentação escolar, dietas específicas (hipertensão, diabetes), público geral, crianças, idosos e atletas.' },
  { title: 'Possibilidade de Adaptação', text: 'Substituição de ingredientes caros por opções locais, redução de sal, açúcar e gordura, troca de ultraprocessados por alimentos in natura e adaptações para restrições alimentares (sem lactose, sem glúten).' },
];
...
function TermoCard({ t, i }) {
  return (
    <motion.div
      ...
      className="group relative flex flex-col overflow-hidden rounded-xl border border-linen-300/80 bg-white/70 p-5 sm:p-6 transition-all duration-300 hover:bg-teal-100 hover:shadow-[0_8px_32px_rgba(32,51,49,0.07)]"
    >
      ...
      <p className="font-tacker text-[1.1rem] sm:text-[1.2rem] font-semibold text-dark mb-2">{t.nome}</p>
      <p className="text-[14px] sm:text-[14.5px] text-dark/50 leading-relaxed flex-1">{t.definicao}</p>
...
function NovaCard({ g, i }) {
  ...
  return (
    <motion.div
      custom={i} variants={rev} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-40px' }}
      className="group relative flex items-start gap-4 sm:gap-5 py-3"
    >
      ...
      <div className="flex-1 pb-2">
        <p className="font-pally text-[1rem] sm:text-[1.1rem] font-normal text-dark mb-1.5 leading-snug">
          {g.titulo}
        </p>
    ...
    <div className="bg-bg">
      ...
      <section className="page-hero -mt-9 border-b-0">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 right-0 w-[280px] h-[280px] sm:w-[420px] sm:h-[420px] rounded-full bg-teal-100/70 blur-[120px]" />
          <div className="absolute bottom-0 left-0 w-[240px] h-[240px] sm:w-[380px] sm:h-[380px] rounded-full bg-slate-100/60 blur-[110px]" />
        </div>
        <div className="page-hero-shell">
          <div className="overflow-hidden mb-1 w-full relative">
            <motion.h2
              ...
              className="font-pally font-semibold text-dark leading-[0.92] flex items-center gap-2 sm:gap-3 flex-wrap"
              style={{ fontSize: 'clamp(28px, 5vw, 86px)' }}
            >
              <FaClipboardList className="bg-teal-400 p-2 sm:p-3 text-white rounded-md shrink-0" />
              Ficha <span className="font-semibold text-stone-500 leading-[0.92]">Técnica</span>
            </motion.h2>
          </div>
        </div>
      </section>
      ...
      {/* ══════════════════════════════════════
          O QUE É — intro block
          ══════════════════════════════════════ */}
      <section className="section-band py-16 sm:py-20 bg-teal-900">
        <div className="section-inner max-w-7xl mx-auto px-4 sm:px-6 md:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start">
            ...
            >
              ...
              {/* quick stats */}
              <div className="mt-6 sm:mt-8 grid grid-cols-3 gap-px bg-stone-200/80 rounded-xl overflow-hidden">
                {[
                  { val: 'FC', label: 'Fator de Correção' },
                  { val: 'IC', label: 'Índice de Cozimento' },
                  { val: 'IH', label: 'Índice de Hidratação' },
                ].map((s, i) => (
                  <div key={i} className="bg-dark/10 px-2 sm:px-4 py-4 sm:py-6 text-center flex flex-col gap-1">
                    <p className="font-pally text-[1.5rem] sm:text-[2rem] font-bold text-teal-600 leading-none">{s.val}</p>
                    <p className="font-mono text-[10px] sm:text-[13px] tracking-widest uppercase text-dark/80">{s.label}</p>
                  </div>
                ))}
              </div>
            </motion.div>
          ...
          <p className="text-[14px] sm:text-[15px] text-dark/60 leading-[1.85] text-justify mt-10 sm:mt-12">
      ...
      {/* ══════════════════════════════════════
          TERMOS — card grid
          ══════════════════════════════════════ */}
      <section className="section-divider-top relative overflow-hidden bg-linen-50 py-16 sm:py-24">
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute right-0 top-0 h-[350px] w-[350px] rounded-full bg-teal-100/35 blur-[110px]" />
        </div>

        <div className="section-inner relative mx-auto max-w-7xl px-4 sm:px-6 md:px-10">
          ...
          >
            <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-linen-300 bg-zinc-200">
              <span className="font-inter font-semibold text-[13px] sm:text-[14px] tracking-wide uppercase text-dark/50">
            ...
            <div className="divide-y divide-linen-300/70">
              {outrosTermos.map((o, i) => (
                <motion.div
                  ...
                  className="group flex items-start gap-3 sm:gap-6 px-4 sm:px-6 py-4 sm:py-5 hover:bg-teal-100 transition-colors duration-200"
                >
                  <span className="font-mono flex justify-center items-center text-[13px] sm:text-[14px] text-dark/50 shrink-0 p-3 sm:p-4 rounded-md bg-teal-400/20">
                    {String(i + 1).padStart(2, '0')}.
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="font-tracker text-[0.95rem] sm:text-[1rem] font-bold text-dark/90">{o.label}</p>
                    <p className="font-inter text-[13px] sm:text-[14.5px] text-dark/48">{o.desc}</p>
                  </div>
                  <div className="h-1 rounded-full w-0 self-center bg-teal-400 transition-all duration-500 group-hover:w-12 shrink-0 hidden sm:block" />
                </motion.div>
              ))}
            </div>
          </motion.div>
      ...
      {/* ══════════════════════════════════════
          CLASSIFICAÇÃO NOVA
          ══════════════════════════════════════ */}
      {/* FIX: removed h-screen + flex-center — it breaks on mobile and causes overflow */}
      <section className="section-divider-top py-16 sm:py-24 bg-teal-700/10">
        <div className="section-inner max-w-7xl mx-auto px-4 sm:px-6 md:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
            ...
            <div className="lg:col-span-5 lg:sticky lg:top-24 lg:self-start">
              ...
              <motion.p
                custom={2} variants={rev} initial="hidden" whileInView="show" viewport={{ once: true }}
                className="font-inter text-[14px] sm:text-[15px] text-justify text-dark/55 leading-[1.85] mb-6 sm:mb-8"
              >
              ...
              <motion.a
                ...
                className="group inline-flex items-center gap-2 font-mono text-[12px] tracking-[0.15em] uppercase text-teal-600 border border-teal-400/40 px-4 py-2.5 rounded-lg transition-all duration-300 hover:bg-teal-100 mb-8"
              >
            ...
            <div className="lg:col-span-6 lg:col-start-7">
              <motion.div
                custom={0} variants={rev} initial="hidden" whileInView="show" viewport={{ once: true }}
                className="rounded-2xl border border-linen-300 bg-white/70 px-5 sm:px-8 py-5 sm:py-6"
              >
                <p className="font-inter leading-1 text-[12px] sm:text-[13.5px] tracking-widest uppercase text-dark/70 font-semibold mb-5 sm:mb-6 pb-4 border-b-2 border-teal-500/40">
                  Classificação NOVA
                </p>
                {gruposNOVA.map((g, i) => (
      ...
      {/* ══════════════════════════════════════
          ...
          ══════════════════════════════════════ */}
      <section className="section-divider-top section-band-soft py-16 sm:py-24">
        <div className="section-inner max-w-7xl mx-auto px-4 sm:px-6 md:px-10">
          ...
          >
            <div className="flex items-center justify-between px-5 sm:px-8 py-5 sm:py-6 border-b border-linen-300 gap-4">
              <div>
                <p className="font-mono text-[9px] tracking-widest uppercase text-dark/25 mb-1">
                  Aula Leguminosas
                </p>
                <h3 className="font-display text-[1.2rem] sm:text-[1.4rem] font-semibold text-dark">
                  Feijoada Vegana
                </h3>
              </div>
              <span className="inline-flex items-center gap-1.5 font-mono text-[10px] tracking-[0.16em] uppercase text-teal-600 shrink-0">
      ...
      {/* ══════════════════════════════════════
          REFERÊNCIAS
          ══════════════════════════════════════ */}
      <section className="section-divider-top py-12 sm:py-16 bg-bg">
        <div className="section-inner max-w-7xl mx-auto px-4 sm:px-6 md:px-10">
          {/* FIX: logic was inverted — openRef now correctly controls visibility */}
          <button
            onClick={() => setOpenRef(v => !v)}
            className="group flex w-full items-center justify-between py-4 cursor-pointer hover:bg-teal-500/20 transition-colors duration-300 px-3 rounded-lg"
          >
            <span className="font-mono text-[13px] sm:text-[14px] font-semibold tracking-widest uppercase text-dark/60">
          ...
          <AnimatePresence>
            {openRef && (
              ...
              >
                <div className="border-t-2 border-linen-300 pt-5 sm:pt-6 pb-2 space-y-3">
                  {refs.map((r, i) => (
                    <p key={i} className="flex items-start text-[13px] sm:text-[14px] text-dark/70 pl-4 border-l border-stone-200">
                      <BsDot className="size-7 shrink-0 mt-0.5" />
                      <span>{r}</span>
                    </p>
                  ))}
                </div>
